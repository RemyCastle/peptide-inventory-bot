...
6. **Payment methods.** `/storefront` exposes `checkout_ready` (true iff ≥1 active method) plus name+type only — no handles, no pay URLs. `payments` is empty if the owner paused every row. Mini App `POST /order` returns 409 `no_payment_methods` instead of creating a code with no rail; stock fail is `sold_out`, below-minimum is `min_order`. Boot seeds Venmo+PayPal on the catalog shop even without `UNICORN_CLAIM_TOKEN`. `GET /order-status` returns the same `payment_methods` objects as checkout (`pay_url` only while `needs_payment`). Seed/edit in Admin → Payments or `/webpanel`, never by resetting the DB.
