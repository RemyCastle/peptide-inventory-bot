...
6. **Payment methods.** `/storefront` `payments` can be empty if none are active. Mini App pay screen then has no rails — seed/edit in `/webpanel`, never by resetting the DB.
