...
6. **Payment methods.** `/storefront` exposes `checkout_ready` (true iff ≥1 **usable** active method — typed rail with a handle, or custom instructions), a buyer `message`, and `invoices_enabled` (boolean) plus name+type only — no handles, no pay URLs, no `pay_hint`. `payments` is empty if the owner paused every row **or** every enabled row has no handle. Mini App `POST /order` returns 409 `no_payment_methods` instead of creating a code with no rail; stock fail is `sold_out`, below-minimum is `min_order`. `/health` `payments.usable` is the usable count (never handles). Boot seeds Venmo+PayPal on the catalog shop even without `UNICORN_CLAIM_TOKEN`. `GET /order-status` returns the same `payment_methods` objects as checkout (`pay_url` + `pay_hint` only while `needs_payment`) plus a status `message`, `can_mark_paid`, and `mark_paid_hint`. Mini App `POST /order-paid` (same initData as checkout) moves `pending_payment` → `awaiting_confirmation` without deducting stock. The vendor PAYMENT CLAIM ping includes `/confirm?ct=` and `/cancel?xt=` URL buttons (vendor bot has no callbacks); `/confirm` success offers `/track?ot=`. Telegram ➕ PayPal / Apple Cash quick-add is wired (`PAY_TPL_CALLBACK_RE` = every `METHOD_TYPES` value). Panel/API 400 if an **enabled** typed method is saved with no target (pause still allowed); 400 copy names the missing field (cashtag / wallet / Zelle email or phone, not a generic "handle"). Panel field labels and placeholders follow `PAYMENT_TARGET_COPY`. Crypto with an address but no network note stays usable and shows an admin `rail_warning`. Telegram TEXT receipts, sale admin reports, payment-claim pings, payment-confirmed DMs, and saved-address previews sanitize buyer/item/ship/pay names the same way HTML receipts and NEW ORDER notify already do. Seed/edit in Admin → Payments or `/webpanel`, never by resetting the DB.
