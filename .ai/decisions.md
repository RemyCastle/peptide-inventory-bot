...
### 2026-08-30
- Decision: Cut Unicorn Magic Factory off SPBC back-room paths without splitting hosts or touching `inventory.db`. Confirmed in code (not assumed): same process/DB can hold an SPBC shop row and a Unicorn shop row. Back room was `order_router.compute_quotes` (quotes every shop except `SPBC_SHOP_CHAT_ID`), `vendor_links` mappings, `collab` guest shares, `vendor_stores` `voffer_*`/`shand_*` + `vendor_bot_for_user` handoff, POST `/order` and NEW ORDER notify falling back to the main SPBC bot, and `site_sync.sync_shop` writing springfieldpbc.com into a shop. Unicorn checkout itself was already local (`db.create_order` + `generate_payment_code` 🎁+6 digits). Isolation helper `unicorn_shop.py` (title / `UNICORN_SHOP_CHAT_ID` / vendor name). Unicorn shops are skipped for quotes, links, collab guest listing, fulfillment apply, vendor-bot handoff, and SPBC catalog sync. Unicorn vendor bot no longer registers SPBC offer/handoff handlers and never falls back to `@SPBCOrderBot`. `run_cloud._run_foreground` keeps HTTP + vendor bots up when `TELEGRAM_BOT_TOKEN` is unset so Unicorn does not require the SPBC main bot. Other vendor shops on this commit (spbc-supplier-bot) still quote/hand off. Did not delete product/order rows, did not change Patriotic Peptides / springfieldpbc.com public catalog / spbc-orders Worker, did not rewrite the Unicorn bot.
- Why: Remy: Ghostie shop stays customer-facing; they do not need SPBC to connect anymore. Same commit auto-deploys to unicornfartzz-bot and spbc-supplier-bot.
- Tests: `tests/test_unicorn_spbc_cut.py`; existing router/links/payables/vendor-webpanel tests retargeted off Unicorn titles so they still cover other vendors
